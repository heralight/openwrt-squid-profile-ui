...
The interface is designed for low-power routers: it keeps the configuration UCI-backed, avoids heavy frontend dependencies, validates Squid before every apply and restarts Squid only after validation succeeds.
...
The LuCI tabs show a `squid_profiles service` indicator so you can see whether the helper is enabled before applying changes.
...
- Shows the real output of `/usr/libexec/squid-profiles validate` and `/usr/libexec/squid-profiles apply` from the LuCI action buttons.
- Restarts Squid with `/etc/init.d/squid restart` after successful validation.
...
The manual Validate and Apply buttons execute the same helper through rpcd. Use Validate first when changing domain rules; any syntax error is shown directly in the LuCI notification.
...
In list mode, enter exact domains or leading-dot wildcard domains. A list value such as `*.kk.com` is normalized to `.kk.com`.

In full-text mode, use one rule per line:
...
Full-text mode stores the text block as `raw_rules` and ignores stale list values. Switching back to list mode parses the text and rewrites the allow/deny lists.
...
5. Click **Validate** to run `/usr/libexec/squid-profiles validate` and inspect the returned output.
...
The Apply action uses the same validation path, so a bad domain such as `'.google.com` blocks the apply and prevents a Squid restart.
...
- The runtime Squid files are generated artifacts; use LuCI, UCI or `/usr/libexec/squid-profiles` as the control plane.
