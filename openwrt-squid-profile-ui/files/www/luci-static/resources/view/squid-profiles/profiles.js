...
function parseRawRules(sectionId) {
    var raw = String(uci.get('squid_profiles', sectionId, 'raw_rules') || '');
    var allow = [];
    var deny = [];
...
function collectDomainSet(sectionId) {
    ...
    if (mode === 'text')
        return parseRawRules(sectionId);

    return {
        allow: optionList(uci.get('squid_profiles', sectionId, 'allow_domain')),
        deny: optionList(uci.get('squid_profiles', sectionId, 'deny_domain'))
    };
}
