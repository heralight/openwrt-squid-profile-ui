...
The package is designed for low-power OpenWrt routers: no external frontend framework, small shell helpers, UCI-backed configuration and explicit validation before Squid is restarted.
...
- Full-text profile mode stores `raw_rules` as the authoritative source; list mode stores `allow_domain` and `deny_domain`.
...
- Global validation and apply actions that execute `/usr/libexec/squid-profiles validate|apply` through LuCI `fs.exec`/rpcd `file.exec`.
- Visible `squid_profiles` service enabled/disabled indicator in every tab.
- Mandatory `squid -k parse` before any Squid restart.
...
Profile edit modes are intentionally exclusive. In `lists` mode, `allow_domain` and `deny_domain` are the editable source. In `text` mode, `raw_rules` is the editable source and the generated domain lists are derived from that text.
...
4. Only then the helper copies changed files with dated backups.
5. OpenWrt's `Save & Apply` triggers the helper through the `squid_profiles` procd reload hook.
6. The helper restarts Squid with `/etc/init.d/squid restart` after validation succeeds.
...
Tagged releases are created from Git tags matching `v*`; the `v1.0` tag publishes the GitHub release for package version `1.0`. If you use tagged releases, you can fetch the latest generated APK directly with `wget`:
...
through the `squid-profiles` init service. The LuCI Validate and Apply buttons call the same helper directly through rpcd `file.exec`, so their notifications show the real stdout/stderr from `/usr/libexec/squid-profiles validate` or `/usr/libexec/squid-profiles apply`.
...
5. Only if validation succeeds, restart Squid:

```sh
/etc/init.d/squid restart
```

If `/etc/init.d/squid` is not available, the helper falls back to `squid -k reconfigure` or starts Squid directly. If validation fails, Squid is not restarted and the full command output is returned to LuCI.
...
The test image also copies a lightweight Squid/init overlay. This keeps functional tests deterministic while still exercising the real OpenWrt container, LuCI-served JavaScript, rpcd `/ubus` calls, UCI writes, generated `/etc/squid/domains/*.txt` files and generated `/etc/squid/maps/10-profiles.conf`.
...
With the Podman test platform running, execute the end-to-end functional checks:

```sh
sh tests/shell/test_platform_functional_checks.sh
```

That test logs into the OpenWrt LuCI/rpcd instance on `localhost:8080`, verifies that `validate` and `apply` actions use `/usr/libexec/squid-profiles`, verifies Save & Apply through `/sbin/reload_config`, and checks the generated Squid domain/map files for the `pc-dev` device policy scenarios.
